...
/**
 * Longest gap we are willing to call a measured decision time.
 *
 * The clock stops when this server first *observes* the run finish, which is
 * only accurate while somebody is polling. If the employee closes the tab the
 * next observation might be an hour later, and recording that as "time to
 * decision" would be a wrong number nobody could spot. Past this ceiling we
 * store nothing and every screen says the run was not timed.
 */
const TIMING_CEILING_MS = 5 * 60 * 1000;
...
/**
 * Stop the decision clock, the first time we see a run finish.
 *
 * Only a run we watched to the end gets a time. An observation that arrives
 * after TIMING_CEILING_MS says nothing about how long the workflow took, only
 * about when somebody next opened the page, so it is left unmeasured rather
 * than recorded as a number the screens would present as fact.
 */
function stopClock(record) {
  const t = record.timing;
  if (!t || !t.decisionStartedAt || t.decidedAt) return;

  const decidedAt = new Date();
  const decisionMs = decidedAt.getTime() - Date.parse(t.decisionStartedAt);
  if (!(decisionMs >= 0)) return;

  t.decidedAt = decidedAt.toISOString();
  if (decisionMs > TIMING_CEILING_MS) {
    t.unmeasured = true;                       // observed too late to mean anything
    return;
  }
  t.decisionMs = decisionMs;
  t.totalMs = (t.uploadMs || 0) + decisionMs;
}
...
app.post('/api/submit', requireSignedIn, upload.single('receipt'), async (req, res) => {
  try {
    ...
    // Two clocks. The upload is measured on its own because it is the part that
    // scales with the size of the receipt rather than with the workflow, and a
    // slow upload should never read as a slow decision.
    ...
    // The decision clock starts the moment Opus accepts the run, not when the
    // form was posted: everything before this is our own upload, already timed.
...
app.get('/api/status/:caseId', requireSignedIn, async (req, res) => {
  try {
    ...
    if (outcome.state === 'done') {
      // applyOutcome has just stopped the clock, so re-read the record rather
      // than timing the run a second time here from a different starting point.
      const saved = await loadRecord(caseId);
      const t = (saved && saved.timing) || {};
      ...
      return res.json({
        ...
        timing: {
          uploadMs: typeof t.uploadMs === 'number' ? t.uploadMs : null,
          decisionMs: typeof t.decisionMs === 'number' ? t.decisionMs : null,
          totalMs: typeof t.totalMs === 'number' ? t.totalMs : null,
        },
      });
    }
    res.json(outcome);
...
function toRow(record) {
  ...
  return {
    ...
    // Milliseconds, or null for a run that was never timed. Every screen that
    // shows a duration reads these three, so the result page, the claims list
    // and the report can never quote different numbers for the same claim.
    uploadMs: typeof t.uploadMs === 'number' ? t.uploadMs : null,
    decisionMs: typeof t.decisionMs === 'number' ? t.decisionMs : null,
    totalMs: typeof t.totalMs === 'number' ? t.totalMs : null,
...
app.get('/api/claims.csv', requireSignedIn, async (req, res) => {
  ...
  try {
    ...
    const columns = [
      ...
      // Seconds, not milliseconds: this column is read in a spreadsheet by
      // someone averaging it, and 20.7 is a number they can work with.
      ['Time to decision (s)', 'totalSeconds'],
      ['Upload (s)', 'uploadSeconds'], ['Decision (s)', 'decisionSeconds'],
      ['Summary', 'summary'], ['Case id', 'caseId'],
    ];

    const secs = (ms) => (typeof ms === 'number' ? (ms / 1000).toFixed(1) : '');
    rows.forEach((r) => {
      r.totalSeconds = secs(r.totalMs);
      r.uploadSeconds = secs(r.uploadMs);
      r.decisionSeconds = secs(r.decisionMs);
    });
