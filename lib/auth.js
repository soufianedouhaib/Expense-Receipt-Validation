...
/**
 * A guest session: signed in enough to file a claim, with no identity of its
 * own. Whoever it is types their name and email on the form, and because the
 * app cannot vouch for either, a guest gets the submit page and nothing else —
 * no claim history, since an email you typed is not an email you proved.
 */
function startGuestSession(res) {
  var payload = {
    email: '',
    name: '',
    guest: true,
