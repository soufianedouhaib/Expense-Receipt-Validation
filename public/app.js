...
(function () {
  ...
  /* The clock stops when the server first sees the run finish, so the poll gap
     is the error bar on every duration this app prints. Most runs land inside a
     minute, so poll every second there and ease off afterwards: the accuracy is
     bought exactly where the number is going to be read. */
  var POLL_FAST_MS = 1000;
  var POLL_FAST_WINDOW_MS = 60 * 1000;
  var POLL_SLOW_MS = 3000;

  function pollGap() {
    return (Date.now() - pollStartedAt) < POLL_FAST_WINDOW_MS ? POLL_FAST_MS : POLL_SLOW_MS;
  }
      ...
      fetch('/api/status/' + encodeURIComponent(caseId))
        ...
        .then(function (r) {
          ...
          if (data.state === 'running') {
            advanceProgress();
            pollTimer = setTimeout(tick, pollGap());
            return;
          }
  ...
  /* How long it took, when we actually measured it.
     A run the server could not time is hidden outright rather than shown as a
     dash: an empty row beside "Time to decision" reads like zero seconds. */
  function renderTiming(timing) {
    var wrap = $('ttd');
    if (!wrap) return;

    var t = timing || {};
    var total = window.fmtDuration ? window.fmtDuration(t.totalMs) : null;
    if (!total) { wrap.hidden = true; return; }

    $('ttd-total').textContent = total;

    /* The split is what makes the number credible: a viewer who sees only
       "20.7s" wonders what is in it. Both halves or neither. */
    var up = window.fmtDuration(t.uploadMs);
    var dec = window.fmtDuration(t.decisionMs);
    $('ttd-break').textContent = (up && dec)
      ? 'receipt uploaded ' + up + '   ·   read, compared and decided ' + dec
      : '';
