...
(function () {
  ...
  function compute() {
    ...
    list.forEach(function (row) {
      ...
      /* The average is over the claims we timed, not over every claim in the
         period. A run nobody watched to the end has no duration, and folding a
         zero in for it would drag the average toward a number no claim took. */
      if (typeof row.totalMs === 'number' && row.totalMs >= 0) {
        totals.speedMs += row.totalMs;
        totals.speedCount += 1;
      }
  ...
  /* Mean time to decision across the timed claims in the period, in ms, or
     null when the period holds nothing we timed. */
  function avgMs(t) {
    return t.speedCount > 0 ? (t.speedMs / t.speedCount) : null;
  }

  function durationText(ms) {
    return (window.fmtDuration && window.fmtDuration(ms)) || '—';
  }
  ...
  function renderRibbon(v) {
    ...
    if (avg === null) {
      $('k-speed').textContent = '—';
      $('k-speed')._value = 0;                 // so the next period counts up from zero
      $('k-speed-sub').textContent = 'none timed';
    } else {
      countTo($('k-speed'), avg, durationText);
      /* "timed" is load-bearing: this count is smaller than the Claims tile two
         columns over whenever a run finished with nobody watching, and without
         the word the ribbon looks like it contradicts itself. */
      $('k-speed-sub').textContent = 'across ' + t.speedCount +
        ' timed claim' + (t.speedCount === 1 ? '' : 's');
    }
  }
  ...
  function panelHeadline(v) {
    ...
    var avg = avgMs(t);
    slot(root, 'speed').textContent = avg === null
      ? 'No claim in this period was timed'
      : 'Average time to decision ' + durationText(avg) +
        ', across ' + t.speedCount + ' timed claim' + (t.speedCount === 1 ? '' : 's');
